...
Todos los parámetros forman parte de un _espectro de respuesta_ y por lo tanto están correlacionados. Las ordenadas de la figura [-@fig-Spectra] muestra los valores de 10 parámetros arbitrarios del dataset, donnde las abscisas presentan la variable continua $f [Hz]$ equivalente al rango discreto $f=100,101 ... 400$. 
...
La tercer estrategia consiste en emplear métodos de regresión que no requieren una formulación paramétrica explícita, y por lo tanto no están condicionados por el número de parámetros. 

En un modelo no-paramétrico, no se establecen restricciones acerca de la función de distribución de la media condicional $E[Y/X]$

En este trabajo, se analizaran los modelos de regresión basados en núcleos (`knn()`), y modelos basados en árboles de decisión como el Random Forest.
Ambas formulaciones encuentran internamente un set de parámetros óptimo y no requieren una etapa previa de pre-selección, aunque sigue siendo posible identificar el grado de explicación de la varianza de los parámetros del dataset.  
Los modelos mencionados son no-paramétricos 
